...
export default function OrderPendingView() {
  ...
  const [loadingCourses, errorCourses, responseCourses] = useFetchData(
    () =>
      axiosClient.get(
        `/v1/get-pending-orders?pageIndex=${searchParams.get('page') ? Number(searchParams.get('page')) : 1}&pageSize=${
  ...
  const handleProgress = async (id) => {
    try {
      setOpen(null);
      await axiosClient.put(`/v1/update-order-to-in-process?id=${id}`);
      refetchApp();
    } catch (e) {
      //   notification.error({ message: 'Sorry! Something went wrong. App server error' })
    }
  ...
  const columns_courses = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <>
          <IconButton onClick={(event) => handleOpenMenu(event, record.id)}>
          ...
          >
            ...
            <MenuItem onClick={() => handleProgress(rowId)}>
